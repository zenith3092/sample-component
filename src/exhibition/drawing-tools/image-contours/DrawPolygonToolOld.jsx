 ...
 * @property {number[][]} polygons - normalized coordinates of polygons
 * @property {(outputWidth?: number, outputHeight?: number, quality?: number) => Promise<File>} outputImageFile - generate image file with polygons
 * @property {(image: string) => void} changeImage - change background image
 * @property {(polygonsArray: number[][]) => void} initPolygons - initialize the normalized coordinates of polygons
 * @property {() => void} clearPolygons - clear polygons
 */
...
const DrawPolygonTool = forwardRef(
    ...
    ({}, ref) => {
        ...
        function strokeVertices(canvas, coordArray) {
            ...
            ctx.beginPath();
            ctx.moveTo(
                coordArray[0] * canvas.width,
                coordArray[1] * canvas.height
            );
        ...
        function getCenterPoint(canvas, coordArray) {
            ...
            for (let i = 0; i < coordArray.length; i += 2) {
                ...
                const x2 =
                    coordArray[(i + 2) % coordArray.length] * canvas.width;
                const y2 =
                    coordArray[(i + 3) % coordArray.length] * canvas.height;
                ...
                >
                    ...
                    <PromptContent>
                        ...
                        >
                            ...
                            <button onClick={() => setOpenConfirmPrompt(false)}>
                                No
                            </button>
                        </div>
                    </PromptContent>
